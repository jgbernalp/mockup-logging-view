...
export const ColumnManagementModal: React.FC<ColumnManagementModalProps> = ({
  ...
  return (
    <Modal
      ...
      actions={[
        <Button key="create" variant="primary" form="modal-with-form-form" onClick={handleColumnsSelected}>
          Save
        </Button>,
    ...
    >
      <DataList aria-label="Simple data list example" isCompact>
        {currentColumns.map((column) => (
          <DataListItem key={column.title} aria-labelledby={`column-${column.title}`}>
            <DataListItemRow>
              <DataListCheck
